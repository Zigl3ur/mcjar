...
func JarHandler(version, build, outPath string) error {
...
func getUrl(version, build string) (string, error) {
	...
	return fmt.Sprintf("%s/%s/%s/download", baseUrl, version, build), nil
}
...
func getBuildList(version string) ([]string, error) {
	...
	if status, err := utils.GetReqJson(fmt.Sprintf("%s/%s", baseUrl, version), &builds); err != nil {
